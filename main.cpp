// Alla includes. iostream behövs för input/output, random och chrono för slumptalsgenerering, fcntl, io och stdio för att korrekt hantera breda karaktärer, variant för datalagring, algorithm för find och string för enkla strängfunktioner
#include <iostream>
#include <random>
#include <chrono>
#include <fcntl.h>
...
void GetUserInput()
{
	...
	do {
		...
		// Omvandla input till enbart små bokstäver för lättare jämförelser
...
int GetRandomNumber(bool returnColor)
{
	if (!returnColor)
	{
...
void PrintOutro()
{
	std::wcout << "Farväl, vi synes åter i hazardspelshallarna.";
}